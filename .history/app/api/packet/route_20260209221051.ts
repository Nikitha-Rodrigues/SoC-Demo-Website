...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'covert') {
      ...
      `
      if (!showAll) query += `\nLIMIT ? OFFSET ?`
      const executed = showAll ? mysql2.format(query, []) : mysql2.format(query, [limit, offset])
      const [rows] = showAll ? await pool.execute(query) : await pool.execute(query, [limit, offset])
      return NextResponse.json({ executed, rows })
    }
    ...
    if (action === 'by_protocol') {
      ...
      if (protocol === 'ALL') {
        ...
        `
        if (!showAll) query += `\nLIMIT ? OFFSET ?`
        const executed = showAll ? mysql2.format(query, [protocol]) : mysql2.format(query, [protocol, limit, offset])
        const [rows] = showAll ? await pool.execute(query, [protocol]) : await pool.execute(query, [protocol, limit, offset])
        return NextResponse.json({ executed, rows })
      }
    }
